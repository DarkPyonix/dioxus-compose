...
| 🌐 Web (wasm) | Boundary works, not yet seen on screen | One `WebAssembly.Memory`, defined by the Kotlin/Wasm module and imported by the Rust one, so a batch is read where it was written and nothing is copied. Renderer to Host calls cross a generated JavaScript forwarder, measured at 12 ns, because a browser will not give you a shared memory and a direct binding at the same time; the other direction has no JavaScript on it. A click reaches the Rust handler and the state change comes back, tested in a browser. Nobody has looked at the pixels yet (`PR-6`, milestone M7) |
