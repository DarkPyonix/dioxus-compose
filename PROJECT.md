...
| Q2 | 서드파티 Compose 컴포넌트를 스키마에 확장하는 방식 | 후보 탐색 중 (SPEC FR-11). **1.0 범위 밖으로 정했습니다(2026-09-22).** 답이 나오지 않아서가 아니라, 없어도 라이브러리가 완결적이기 때문입니다. 어휘는 FR-15가 닫아 두었고 확장은 그 위에 얹는 일이므로, 설계가 덜 된 것을 1.0에 넣어 굳히는 것보다 1.1에서 제대로 하는 편이 낫습니다. |
