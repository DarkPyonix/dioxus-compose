...
- **디스크는 실제 비용이지만 생각보다 작았습니다.** 2026-09-22 측정으로 워크트리 하나가 처음부터 빌드해서 1.0GB, 시간은 23초입니다. 같은 시점에 워크트리 여덟 개가 같이 쓰던 빌드 디렉터리는 16GB였습니다. 공유 디렉터리는 워크트리 여덟 개분의 핑거프린트를 한꺼번에 들고 있고 아무도 치우지 않으므로, 이 기계에서는 나누는 쪽이 디스크도 덜 씁니다. 그래도 워크트리를 적게 유지하고, `scripts/setup-worktrees.sh`가 워크트리별 빌드 디렉터리 크기와 남은 공간을 보고해서 349GB 사건이 사후가 아니라 사전에 보이게 합니다.
...
- 워크트리마다 자기 `target/`을 가지므로 체크아웃 하나가 약 1GB를 차지합니다. 열어 둔 워크트리 수가 곧 디스크 비용이고, 그것이 D13이 공유 대신 받아들인 값입니다.
