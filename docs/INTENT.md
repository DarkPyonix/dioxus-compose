...
- **디스크는 실제 비용입니다.** 그래서 워크트리를 적게 유지하고, `scripts/setup-worktrees.sh`가 워크트리별 빌드 디렉터리 크기와 남은 공간을 보고합니다. 349GB 사건이 사후가 아니라 사전에 보이는 것이 공유로 얻던 것의 대체물입니다.
...
- 워크트리마다 자기 `target/`을 가지므로 체크아웃 하나가 수 GB를 차지합니다. 열어 둔 워크트리 수가 곧 디스크 비용이고, 그것이 D13이 공유 대신 받아들인 값입니다.
