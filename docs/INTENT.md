...
**`adaptive`는 Liquid Glass를 고릅니다.** 처음에는 Cupertino를 고르게 두었고, 근거는 두 언어 모두 그 플랫폼의 것이므로 둘 중 하나를 기본으로 정하는 일은 앱이 할 결정이라는 것이었습니다. 그 근거가 `adaptive`가 무엇을 약속하는지와 어긋납니다. `adaptive`는 "앱이 고르지 않는다, 기계가 실제로 쓰는 것을 따른다"는 선언이고, macOS 26과 iOS 26의 기계가 실제로 쓰는 것은 Liquid Glass입니다. Cupertino를 고르는 `adaptive`는 앱이 고르지 않았는데도 한 세대 전의 언어를 고른 결과를 줍니다.

참조 문서가 같은 말을 이미 하고 있었습니다. 계산기 샘플은 `adaptive` 샘플이고 참조 셋 중 mac 자리에 놓인 그림이 macOS 26의 Liquid Glass 계산기입니다. Cupertino를 고르는 한 그 그림은 나올 수 없으므로, 결정과 참조 중 하나가 틀린 상태였습니다.

Cupertino는 사라지지 않습니다. `Theme::unified(DesignSystem::Cupertino)`로 이름을 대고 고르며, 크로스플랫폼 앱이 "iOS처럼" 보이려고 실제로 고르는 어휘라는 위의 판단도 그대로입니다. 이름을 대야 하는 쪽이 둘 중 어느 것이냐가 바뀌었을 뿐입니다. 이 선택은 되돌릴 수 있습니다. 기본값을 바꾸는 일은 태그를 바꾸는 일이 아니기 때문입니다.
...
**실행 파일 하나를 목표로 정합니다.** 번들(msix, dmg, AppImage)로 사용자 눈에 하나로 보이게 하는 것은 이 목표를 대신하지 않습니다. 그것은 별개의 작업이고 둘 다 합니다.

네 가지가 필요하고, 넷 다 같은 문장을 다르게 말한 것입니다. **아카이브가 이미지 안에 있는 것과, 런타임이 "그건 이미 안에 있다"고 아는 것은 다른 일입니다.**

1. **렌더러를 정적 라이브러리로.** native-image는 공유 라이브러리와 실행 파일만 냅니다. 정적 라이브러리 출력 요청([oracle/graal#3053](https://github.com/oracle/graal/issues/3053))은 2020년에 열려 아직 열려 있고, 그 이슈에 우리와 같은 사람이 있습니다. `--native-compiler-path`로 링커 호출을 가로채 같은 재배치 오브젝트를 아카이브로 묶는 우회가 있고 Graal 25에서 동작한다고 보고돼 있습니다.
2. **Skia를 이미지 안으로.** 아카이브는 만들었고 링크도 됩니다. 막힌 곳은 skiko의 데스크톱 로더가 `System.loadLibrary`가 아니라 절대 경로로 `System.load`를 부른다는 것입니다. 빌트인 라이브러리 등록은 앞의 호출에만 답합니다.
3. **AWT를 이미지 안으로.** NIK이 macOS에서 해 주는 것을 Windows에서 해야 합니다. `lib/static/windows-amd64/`에 `.lib` 52개가 있고, `Toolkit.loadLibraries`의 `System.loadLibrary("awt")`가 막는 곳입니다.
4. **ICU 데이터.** `icudtl.dat` 10MB는 파일이라 링킹으로 흡수되지 않습니다. macOS Skia 빌드는 ICU를 `libicu.a`로 품고 있어 이 파일이 아예 없으므로, Skia 빌드 설정의 문제입니다.

**구조는 뒤집지 않습니다.** native-image가 실행 파일을 만들고 Rust를 그 안에 링크하는 쪽이 더 곧은 길이지만, 그러면 앱을 빌드하는 사람이 native-image를 돌려야 하고 "설치는 `Cargo.toml` 한 줄"(D10)이 깨집니다. 1번의 우회가 있는 한 그 값을 치를 이유가 없습니다.
...
| 워크트리 전체가 `target/` 하나를 공유 | Cargo가 path 패키지의 유닛 해시에 경로를 넣지 않아, 내용이 같은 두 워크트리가 캐시 항목 하나가 됩니다. 컴파일 시점에 박힌 절대 경로가 먼저 빌드한 쪽 것이 되어 한 체크아웃의 빌드가 다른 체크아웃의 파일을 말없이 덮어씁니다 (D16) |
