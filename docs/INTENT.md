...
### D12. Liquid Glass는 Apple 자리를 **대체**한다. 와이어 enum에 나란히 놓지 않는다

macOS 26과 iOS 26에서 Apple의 디자인 언어가 Liquid Glass로 바뀌었습니다. 그때까지 Apple 자리를 채우던 평면 Cupertino 룩은 더 이상 어떤 Apple 플랫폼의 현재 모습도 아닙니다. 여기서 선택지는 둘이었습니다.

1. `DesignSystem` enum에 `LiquidGlass` 변형을 하나 더하고, 기존 `Cupertino`를 이전 세대 표현으로 남깁니다.
2. `DesignSystem::Cupertino`가 가리키는 토큰 테이블과 컴포넌트 규칙을 Liquid Glass로 바꿉니다. 변형은 늘지 않습니다.

**2번을 고릅니다.**

첫째, `Cupertino`는 세대 이름이 아니라 **자리 이름**입니다(SPEC 14.1-2). 크로스플랫폼 툴킷에서 "Apple 스타일 위젯 집합"을 가리키는 관례적 이름이고, Apple 자신은 디자인 언어에 제품명을 붙이지 않습니다. 그 자리가 지금 무엇으로 그려지는지는 자리 이름이 답할 문제가 아닙니다. 이름을 `LiquidGlass`로 바꾸면 Apple이 다음에 언어를 또 바꿀 때 같은 결정을 다시 해야 하고, 그때는 와이어 태그가 걸린 이름을 바꾸는 일이 됩니다.

둘째, 변형을 하나 더하면 **폐기된 디자인 시스템 하나를 영구히 유지해야 합니다.** FR-14.1이 약속하는 비용("시스템 하나 = 테이블 하나 + 규칙 구현 하나")은 더하는 쪽에만 적용되는 것이 아니라 남겨 두는 쪽에도 그대로 붙습니다. 위젯 어휘가 늘 때마다 아무도 원하지 않는 2024년 iOS 룩을 위해 규칙을 하나씩 더 써야 합니다. 그 유지 비용을 정당화하려면 "이전 Apple 룩을 골라서 쓰고 싶다"는 요구가 있어야 하는데, 이 프로젝트의 목적(네이티브 플랫폼을 따라가는 것)과 정반대입니다. 과거 버전 룩이 필요한 앱은 `Theme::unified`로 다른 시스템을 고르면 됩니다.

셋째, 나란히 놓으면 **`adaptive`가 무엇을 고를지 답이 하나뿐입니다.** macOS와 iOS는 Liquid Glass를 고릅니다. 그러면 `Cupertino`는 adaptive에서 영원히 선택되지 않는 변형이 되고, 와이어에 실릴 일이 거의 없는 태그가 프로토콜에 남습니다.

**와이어 태그는 그대로입니다.** `Cupertino = 2`는 바뀌지 않습니다(SPEC 20.1). 바뀌는 것은 그 태그가 Renderer 안에서 해석되는 방식뿐이며, 이것이 FR-14.1의 추상화가 실제로 성립한다는 증거이기도 합니다. 디자인 언어가 통째로 바뀌었는데 위젯 정의, Modifier 스키마, Property 스키마, 와이어 포맷 어느 것도 움직이지 않았습니다.

**대체는 정직한 범위 안에서만 합니다.** SPEC 14.1-2가 나눈 두 범위 중 1번(우리가 그린 콘텐츠 위의 재질, 동심 곡률, 가장자리 하이라이트, 겹침으로 표현한 깊이)만 구현하고, 창 뒤 시스템 배경을 비추는 2번은 하지 않으며 했다고 말하지도 않습니다.

### D12-1. 유리는 크기 클래스에 따라 붙는 자리가 다르다
...
### D12-2. 벤더가 한 역할에 두 값을 규정한 경우에만, 컴포넌트 규칙이 고른다
