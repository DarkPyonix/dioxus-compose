...
#### 14.9 iOS 26의 네이티브 Liquid Glass 셸 (`Draft`)
...
- 창의 루트 뷰 컨트롤러가 `UITabBarController`가 됩니다. 목적지 하나가 `UITabBarItem` 하나이고, 라벨은 `Text`(태그 1), 그림은 `Icon`(태그 60)의 `IconRole`을 SF Symbol 이름으로 옮긴 것입니다.
- 선택된 탭의 화면은 **여전히 Compose가 그립니다.** 바뀌는 것은 막대뿐이고, `Navigation`의 내용 자식들은 지금과 같은 인터프리터를 지나 같은 트리로 그려집니다.
- 각 탭의 내용은 `UINavigationController`에 들어갑니다. 시스템이 유리를 입히는 자리가 탭 바와 내비게이션 바 둘이기 때문입니다.
