...
- **트리가 `TopAppBar`로 시작하면** Renderer가 그것을 캡션 영역으로 인식해 시스템 버튼을 피해 배치하고 드래그 영역으로 만듭니다. 그렇지 않으면 콘텐츠 전체에 인셋을 적용합니다. "루트 자식"이 아니라 "트리가 그것으로 시작한다"인 이유는 앱이 막대를 루트에 두지 않기 때문입니다. 화면인 `Column` 안 첫 자식에 둡니다. 그래서 Renderer는 레이아웃 컨테이너의 첫 자식을 따라 내려가며 찾습니다. `Navigation`에서는 멈춥니다. 레일이나 서랍이 있으면 막대가 창의 앞쪽 가장자리에 닿지 않으므로 캡션이 될 수 없고, 콘텐츠가 인셋을 그대로 받습니다.
