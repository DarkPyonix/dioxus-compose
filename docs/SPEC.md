...
- **바가 캡션을 받으면 그 표면이 캡션 띠를 덮습니다.** 인셋을 바 위쪽에 두면 창 맨 위에 페이지 색 띠가 남고, 그것은 "콘텐츠가 타이틀바 아래로 확장된 창"이 아니라 "아무도 요청하지 않은 타이틀바가 있는 창"으로 읽힙니다. macOS 26이 타이틀바 영역과 툴바를 하나의 유리로 그리는 것과 같은 이유입니다(FR-14.1-3). 그래서 캡션 높이와 버튼 폭은 바의 장식 **안쪽** 패딩이 되고, 바는 창 맨 위에서 시작합니다.
