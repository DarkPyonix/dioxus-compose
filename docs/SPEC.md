...
### FR-18 포인터 제스처 (`Agreed`, 미구현)

**2026-09-22 확인: 이 요구사항은 코드에 하나도 들어와 있지 않습니다.** 아래 표의 여덟 이벤트 중 스키마에 있는 것은 없고, 렌더러에 제스처 인식기도 없습니다. 자리는 비워 두었습니다. `ValueChanged`가 태그 16을 쓰는 것이 그 흔적이고, 그래서 나중에 채워도 이미 배정된 태그는 움직이지 않습니다.
...
**검증 방식에 관하여(2026-09-22):** 규약이라 테스트로 지킬 수 있는 것이 아니고, 검토로 지킵니다. 이 항목에 테스트가 없는 것은 빠뜨린 것이 아닙니다.
