...
- **바의 내용은 버튼 띠에 들어가면 같은 줄에, 안 들어가면 그 아래 줄에 놓입니다.** 원래 규칙은 "항상 같은 줄"이었고 근거는 macOS 26이 툴바를 신호등과 같은 선에 둔다는 것이었습니다. 그 근거가 틀렸습니다. macOS 26은 툴바가 있는 창에서 **신호등을 내려서** 같은 선을 만드는데, 그것은 `NSWindow.toolbarStyle`이고 D15가 정리한 대로 여기서 닿을 수 있는 것은 AWT 클라이언트 속성 여덟 개뿐이며 그 목록에 없습니다. 버튼을 못 내리는데 내용만 바의 중앙에 두면 어긋납니다. 2026-09-24 계산기 실측으로 신호등 중심은 창 위에서 14.5px, 제목 중심은 29.5px, **15px 차이**였습니다.
  - 들어가는 경우: 띠 안에서 세로 중앙. 버튼과 같은 선입니다.
  - 안 들어가는 경우: 띠 바로 아래에서 시작합니다. 바는 띠 높이만큼 더 높아지고, 띠는 버튼만 있는 줄이 됩니다. Windows 11이 전체적으로 쓰는 배치이고, AppKit의 `expanded` 툴바 스타일이 하는 일과 같습니다.
  - 버튼이 차지하는 폭은 여전히 앞쪽 패딩입니다.
