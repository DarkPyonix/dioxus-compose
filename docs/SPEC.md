...
- **Lottie는 위젯이 아니라 에셋입니다.** `AssetKind`에 값을 하나 더하고 JSON 바이트를 등록해
  id로 참조합니다. `Png`, `Svg`와 같은 부류이고 새 전달 수단이 필요 없습니다. 태그는 추가
  전용이므로 나중에 더하는 데 제약이 없습니다.
...
- **그리는 방법은 플랫폼이 답합니다.** 데스크톱과 iOS와 웹은 Compose가 skiko를 쓰고 skiko 안에
  skottie가 있으므로 그것으로 그립니다. **Android는 다릅니다**: 우리 Android 렌더러는 skiko가
  아니라 androidx Compose이고 skottie가 없으므로, Compose용 Lottie 라이브러리를 Maven 좌표로
  선언해야 합니다. 남의 라이브러리를 좌표로 말하는 것은 PR-5가 이미 하는 일입니다.

**정해야 하는 것은 설계가 아니라 비용입니다.**

`libskottie.a` 4.7MB와 그 씬 그래프인 `libsksg.a` 3.9MB, 합쳐서 **8.6MB**입니다(2026-09-23
실측, Skia m144 macOS arm64). 렌더러는 애플리케이션마다 빌드되는 것이 아니라 미리 만들어
배포하는 아티팩트 하나이므로, 코어에 넣으면 **Lottie를 쓰지 않는 모든 애플리케이션이 8.6MB를
냅니다.** FR-11의 확장 패키지로 빼도 같습니다. 확장도 결국 같은 렌더러 바이너리에 들어갑니다.

| 선택지 | 크기 | 쓰는 쪽 |
|---|---|---|
| 코어에 넣는다 | 모든 앱이 +8.6MB | 선언 한 줄 |
| 넣지 않는다 | 0 | Lottie를 쓸 수 없음 |
| 렌더러를 두 벌 배포한다 | 고른 쪽만 | 빌드와 배포와 문서가 두 배 |

**지금 상태**: 아무도 요청한 적이 없고, 이미지에서 skottie 심벌이 참조되는지도 아직 확인하지
않았습니다. 요청이 오거나, 참조되지 않는 것이 확인되어 8.6MB를 실제로 덜어낼 수 있다는 것이
확실해지면 그때 위 표에서 고릅니다.
...
- 크기 클래스도 같은 규칙으로 계산해 실어 보냅니다. 노드에게도 "좁다"와 "넓다"는 창에서와 같은 뜻입니다.
