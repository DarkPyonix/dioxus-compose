...
### FR-20 반응형 레이아웃 기반 (창 크기 클래스) (`Agreed`)
...
- 첫 측정에서는 직전 값이 없으므로 한 번 보냅니다. 이것이 Host가 실제 창 크기를 알게 되는 시점이고, 그 전까지 Host의 값은 `Compact` 기본값입니다.
...
2. 300dp에서 599dp까지 같은 클래스 안에서 크기를 바꾸는 동안 이벤트가 **한 번도** 발생하지 않습니다.
3. 28바이트 고정 레코드가 Rust와 Kotlin 양쪽에서 같은 바이트로 인코딩되고 디코딩됩니다(프로토콜 벡터).
4. `use_window_size()`를 쓴 컴포넌트가 클래스 전환에서 다시 렌더링되고, 그렇지 않은 형제는 다시 렌더링되지 않습니다.
5. 알 수 없는 `size_class` 값은 `ProtocolError`가 되고 프로세스가 죽지 않습니다.
