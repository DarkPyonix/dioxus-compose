...
- **전면 시각 요소도 캡션을 받습니다.** 트리의 앞쪽 가장자리가 `Image`이거나 자기 배경을 칠한 노드이면, 바와 같은 자격으로 캡션 띠를 가져갑니다. 창 맨 위를 사진이나 색면이 채우는 화면에서 그 위에 인셋만큼의 빈 띠를 남기면, 바가 캡션을 받지 못했을 때와 똑같이 "아무도 요청하지 않은 타이틀바"로 읽힙니다. 창 버튼은 그 위에 그대로 그려지므로 가려지지 않습니다.
...
- 현재 상태: 19.4의 2번 중 **최소화·최대화·닫기와 캡션 드래그는 동작하고, 가장자리 크기 조절은 아직 없습니다.**
- 현재 상태: 19.3의 `with_window`는 **아직 구현되지 않았습니다.** `LaunchBuilder`가 받는 것은 `mode`와 `theme`뿐이고, 렌더러의 `chrome` 인자에 기본값 말고 다른 값을 넘기는 호출자가 없어서 `Chrome::System`은 도달할 수 없습니다. 이것은 경계 함수 `dioxus_compose_renderer_run`의 인자가 늘어나는 변경이므로 PR-2에 걸리고, 따로 다룹니다. 19.4의 5번은 그때까지 통과하지 않습니다.
