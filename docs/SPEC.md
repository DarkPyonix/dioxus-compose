...
크레이트를 의존성으로 추가한 애플리케이션이 링크에 성공하고 실행에 실패하면 설치가 끝난 것이 아닙니다. Cargo는 의존성 빌드 스크립트의 링크 탐색 경로와 링크 라이브러리는 최종 바이너리까지 넘기지만 **링크 인자는 넘기지 않습니다.** 그래서 다음 두 가지는 크레이트 안에서 해결해야 합니다.

- **macOS 라이브러리 이름.** 아티팩트는 `@rpath/libdioxus_compose_renderer.dylib`라는 이름을 달고 나옵니다. `@rpath`는 그것을 적재한 쪽의 rpath로 풀리는데, 이 크레이트에 의존하기만 한 애플리케이션에는 rpath가 없습니다. 푸는 시점에 캐시 경로를 이름으로 박아 넣어 조회를 절대 경로로 만듭니다. 캐시 항목은 버전과 타깃으로 키가 잡혀 있고 그 라이브러리가 계속 있을 자리이므로, 절대 경로가 과하게 구체적인 것이 아닙니다.
...
Linux는 손댈 것이 없습니다. 배포되는 공유 객체에는 SONAME이 없고 `$ORIGIN` 런패스가 붙어 있어, 링커가 연 경로를 그대로 기록하고 심은 옆에 있는 이미지를 찾습니다.

**Windows는 아직 남아 있습니다.** DLL은 파일 안의 이름이 아니라 로더의 탐색 경로로 찾으므로, 위의 macOS 방식이 통하지 않습니다. 지금은 렌더러 디렉터리가 `PATH`에 있거나 파일이 실행 파일 옆에 있어야 합니다. 이것을 없애려면 빌드 스크립트가 `bin/`을 실행 파일 옆으로 복사해야 하고, 그러면 프로파일마다 수십 MB가 복사됩니다. **Windows 기계에서 확인할 수 있을 때 결정합니다.** 확인 없이 복사부터 넣으면 검증되지 않은 수십 MB짜리 동작이 모든 빌드에 들어갑니다.
