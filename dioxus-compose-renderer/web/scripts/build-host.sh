...
# Decided before the environment is set, because what follows RUSTFLAGS has to be the
# command itself.
if [[ -n "$sample" ]]; then
...
if [[ -n "$sample" ]]; then
    built="$target_dir/$target/$profile_dir/sample_${sample//-/_}.wasm"
else
...
# The same module beside the test page. The page a Kotlin/Wasm test runs on is generated by
# the toolchain, so the only way to give a test the Host is to put it in the tree the
# toolchain serves, and `testResources` is the one it copies.
test_destination="$module_dir/testResources/dioxus_compose_host.wasm"
mkdir -p "$(dirname "$test_destination")"
cp "$built" "$test_destination"
