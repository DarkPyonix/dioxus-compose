...
rm -rf "$PROJECT_DIR/build/tasks/_macos_compile${amper_platform}Debug"
...
klib="$PROJECT_DIR/build/tasks/_macos_compile${amper_platform}Debug/macos.klib"
[[ -f "$klib" ]] || die "the compiler produced no klib at $klib" \
    "Expected the :macos:compile${amper_platform}Debug task to run." \
    "Full log: $build_log"
