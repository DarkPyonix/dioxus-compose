 ...
 */
class BreezeDesignSystemTest {

    private val schemes = listOf(BreezeDesignSystem.Light, BreezeDesignSystem.Dark)
    ...
    fun fr14_dark_scheme_is_not_the_light_scheme() {
        val differing = ColorRole.entries.count {
            BreezeDesignSystem.Light.color(it) != BreezeDesignSystem.Dark.color(it)
        }
        assertTrue(differing >= ColorRole.entries.size / 2, "only $differing colour roles change between schemes")
        assertTrue(BreezeDesignSystem.Dark.isDark)
        assertTrue(!BreezeDesignSystem.Light.isDark)
    }

    @Test
    fun fr14_space_ladder_grows_with_each_step() {
        val ladder = SpaceRole.entries.map { BreezeDesignSystem.Light.space(it).value }
    ...
    fun fr14_type_ladder_shrinks_from_display_to_caption() {
        val system = BreezeDesignSystem.Light
