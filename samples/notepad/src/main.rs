...
mod tests {
    ...
    /// The editor under every design system, and in all three windows under the two it was
    /// drawn from.
    ///
    /// A notes app is two references, not one: the iOS notes screen is a document with a
    /// formatting sheet over it, and Microsoft Loop is a sidebar of pages beside one. They
    /// are the same application at two widths, which is exactly the claim this sample
    /// makes, so both are recorded.
    #[test]
    fn fr20_the_editor_is_recorded_in_every_window() {
        ...
        for system in [DesignSystem::Cupertino, DesignSystem::Fluent] {
            record_every_width(directory.as_deref(), "Notepad", system, ColorScheme::Light);
        }
    }
